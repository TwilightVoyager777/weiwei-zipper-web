  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-lg text-gray-600 mb-10">{contactContent.subtitle}</p>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="space-y-6">
            <div className="bg-blue-900 text-white rounded-lg p-6">
              <h2 className="text-lg font-bold mb-4">{contactContent.cardTitle}</h2>
              <div className="space-y-4 text-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-blue-100 space-y-1">
                    <p>{siteBrand.currentAddress}</p>
                    {siteBrand.historicalAddressNote ? (
                      <p className="text-xs text-blue-200">{siteBrand.historicalAddressNote}</p>
                    ) : null}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <a href={`mailto:${siteBrand.email}`} className="text-blue-100 hover:text-white">{siteBrand.email}</a>
                </div>
                <div className="flex items-center gap-3">
                  <svg className="w-5 h-5 flex-shrink-0 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                  <a href={`tel:${siteBrand.phone}`} className="text-blue-100 hover:text-white">{siteBrand.phone}</a>
                </div>
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-blue-100">{siteBrand.businessHours}</span>
                </div>
              </div>
            </div>

            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <p className="text-sm text-green-800 font-medium">{contactContent.responseNote}</p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 text-center">
              <div className="flex items-center justify-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-gray-900">{contactContent.wechatLabel}</h3>
              </div>
              <Image
              ...
              />
              <p className="text-xs text-gray-500 mt-2">{contactContent.wechatScan}</p>
              <p className="text-xs text-gray-400 mt-1">微信号：{siteBrand.wechatId}</p>
            </div>
          </div>
