          ...
          <div className="order-2 space-y-4 sm:space-y-6 lg:order-1">
              ...
              <div className="space-y-4 text-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-blue-100 space-y-1">
                    <p>{brand.currentAddress}</p>
                    {brand.historicalAddressNote ? (
                      <p className="text-xs text-blue-200">{brand.historicalAddressNote}</p>
                    ) : null}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <a href={`mailto:${brand.email}`} className="text-blue-100 hover:text-white">{brand.email}</a>
                </div>
                <div className="flex items-center gap-3">
                  <svg className="w-5 h-5 flex-shrink-0 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                  <a href={`tel:${brand.phone.replace(/\s+/g, '')}`} className="text-blue-100 hover:text-white"><span dir="ltr">{brand.phone}</span></a>
                </div>
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-blue-100">{brand.businessHours}</span>
                </div>
              </div>
            </div>

            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <p className="text-sm text-green-800 font-medium">{content.responseNote}</p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 text-center">
              <div className="flex items-center justify-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-gray-900">{content.wechatLabel}</h3>
              </div>
              <Image
                src={brand.wechatQrPath}
                alt={content.wechatScan}
                width={180}
                height={180}
